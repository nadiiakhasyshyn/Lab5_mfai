...
def fitness(schedule, num_lessons, max_lessons_per_day):
    ...
    for class_schedule in schedule:
        ...
        # Перевірка вчитель відповідає предмету та спеціальний кабінет
        for day in class_schedule:
            for lesson in day:
                if lesson is not None:
                    if lesson.name not in lesson.teacher.lessons:
                        fitness_score -= 10
                    if lesson.special_room_required and lesson.room != lesson.name + " Room":
                        fitness_score -= 10
...
def print_schedule(schedule, num_days, max_lessons_per_day, num_classes, class_names):
    table = PrettyTable()
    table.field_names = ["Day", "Lesson Number"] + class_names[:num_classes]
...
def generate_data():
    num_lessons = random.randint(23, 25)
    num_classes = 2
    num_days = 5
    max_lessons_per_day = 5

    # Define the list of subjects
    subjects = ["Math", "Science", "History", "English", "Music", "PE", "Choreography"]
    ...
    # Assign subjects to the first three teachers
    for i in range(3):
        num_subjects = 4
        teacher_subjects = random.sample(subjects, num_subjects)
        teachers[i].lessons = teacher_subjects

    # Assign subjects to the fourth teacher
    num_subjects = 2
    teacher_subjects = random.sample(subjects, num_subjects)
    teachers[3].lessons = teacher_subjects
...
def main():
    choice = input("Виберіть 'generate', щоб згенерувати дані, або 'read', щоб зчитати дані з файлу: ")
    input_file = "input_file"

    if choice == 'generate':
        num_lessons, num_classes, num_days, max_lessons_per_day, teachers, room_names, class_names, lesson_names = generate_data()
    elif choice == 'read':
    ...
    write_data = input("Виберіть 'yes', щоб записати ці вхідні дані у файл, або 'no', щоб не записувати: ")
    if write_data == 'yes':
        # Запис даних у файл
